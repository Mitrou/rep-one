...
# fp.set_preference('browser.download.dir', "C:\\Git\\rep-one\\file_download\\file") # home setting
fp.set_preference('browser.download.dir', "D:\\myGit\\rep-one\\file_download\\file")   # office PC setting
...
for root, dirs, filenames in os.walk(dwnld_fle_path):
    for filename in filenames:
        ...
        if filename.__contains__("source") or filename.__contains__("target") or filename.__contains__("zip"):
...
filename = max(glob.iglob('%s/*.zip' % dwnld_fle_path), key=os.path.getctime)
os.rename(filename, '%s/target.csv.zip' % dwnld_fle_path)
