...
select	t2.[Plant_Number] 
	...
	FROM .[dbo].[Orders] as t1
		left join [dbo].[OrderLines] as t2 on (t1.[Order_Id] = t2.[Order_Id])
		left join [dbo].[MasterAccounts] t3  on (t1.[MasterAccount_id] = t3.[MasterAccount_id])
		left join [dbo].[SiteAccounts] t4  on (t1.[SiteAccount_id] = t4.[SiteAccount_id])
	WHERE
		1=1
...
/*
RECENT:
update top (30) [dbo].[OrderLines]
	set offhire_date = '2018-04-05'
	where [Order_Id] in (select [Order_Id] from [dbo].[Orders] where MasterAccount_id = '218786') and [OrderLines].[OffHire_Reference] is not null;

NEARING:
update top (4) [dbo].[OrderLines]
	set expectedOffhire_date = '2018-04-07'
	where [Order_Id] in (select [Order_Id] from [dbo].[Orders] where MasterAccount_id = '218786') and [OrderLines].[OffHire_Reference] is null;
